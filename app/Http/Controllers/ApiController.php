        ...
        if ($validator->fails()) {
            ...
            if (count($data->subject) > 0) {
                foreach ($data->subject as $datum) {
                    ...
                    if (count((array)Chapter::where('subject_id', $datum->id)->first()) > 0) {
                        $next = 'Chapter';
        ...
        if ($validator->fails()) {
            ...
            if (count($subjectChapter) > 0) {
                foreach ($subjectChapter as $item) {
                    ...
                    if (Topic::where('chapter_id', $item->id)->first() != NULL) {
                        $next = 'Topic';
                ...
                foreach ($chapterTopic as $item) {
                    ...
                    foreach ($item->document as $docItem) {
                        ...
                        elseif ((in_array('Audio', $audioType) && in_array('Video', $audioType))) {
                            ...
                            $type = (count($audioType)>0)?$audioType[0]:'';
                        }
                    ...
                    if (!empty($course['subject'])) {
                        ...
                        foreach ($course['documents'] as $document) {
                            ...
                            elseif ((in_array('Audio', $audioType) && in_array('Video', $audioType))) {
                                ...
                                $type = (isset($audioType[0]))?$audioType[0]:[];
                            }
    ...
    public function userRegistration(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $otp = $this->core->otpGenerate();
                if(strtolower($request->type)=='email'){
                    $sendSms = $this->core->sendEMAIL($request->email, $otp);
                }
                else{
                    $sendSms = $this->core->sendSMS($request->mobile, $otp);
                }

                if (isset($sendSms->errors) || $sendSms=0) {
...
$data['slider'] = DB::table('home_images')->select('image')->where('delated_at',null)->get();
        ...
        if (count($allCourses) > 0) {
            ...
            foreach ($allCourses as $course) {
                if (!empty($course['subject'])) {
                    foreach ($course['subject'] as $subject) {
                        if (count((array)Chapter::where('subject_id', $subject['id'])->first()) > 0) {
                            $next = 'Chapter';
                        } elseif (Document::where('subject_id', $subject['id'])->count() > 0) {
                            $next = 'document';
                        } else {
    ...
    // dd($doc);
    if(count((array)$doc->course)>0 && count((array)$doc->subject)>0){
        if($doc->course->type=='Free'){
    ...
    else if(count((array)$doc->course)>0){
        return response()->json(['status' => 200, 'data' => $doc->course->type], '200');
    }else{
            return response()->json(['status' => 200, 'data' => $doc->subject->type], '200');
        }
                ...
                if (isset($documentMedia->media) && count($documentMedia->media) > 0) {
                    ...
                    foreach ($documentMedia->media as $item) {
                        ...
                        if ($item->doc_type == 'Video') {
                            $video_url = $this->youtubeDownload($item->doc_file);
                            $video = isset($video_url) ? $video_url : $item->doc_file;
                            $orignal_link = isset($video_url) ? $item->doc_file : '';
                            $doc_type = 'Video';
                        }
                    ...
                    $finalData = [
                        ...
                        'audio_duration' => isset($audioDuration) ? $audioDuration : '',
                        'video_link' => isset($video) ? $video : ($video = ''),
                        'orignal_link'=>isset($orignal_link)?$orignal_link:'',
                        'text_link' => isset($text) ? $text : ($text = ''),
                        'image' => cloudUrl($document->preview_image),
                        'author_name' => $document->author_name,
                        'is_liked' => (!empty($document->likes_id) ? true : false),
    ...
    public function getBooks(){
            ...
            $data=Book::where('status',1)->get(['id','title','doc_type','description','orderNo','preview','file_detail','created_at','author_name']);
            $book=[];
    ...
    public function getBooksWeb(){
          
            $data=Book::where('status',1)->orderBy('orderNo')->get(['id','title','doc_type','description','orderNo','preview','file_detail','created_at','author_name']);
            $book=[];
            foreach($data as $k=>$item){
                $book[$k]=$item;
                $book[$k]['preview']=cloudUrl($item->preview);
                ...
                if(isset($item->doc_type) && $item->doc_type=='text'){
                    $book[$k]['file_detail']=substr(strip_tags($item->file_detail),0,110) . "...";
                }
            }
            if(count($book)>0){
                return response()->json(['status' => 200, 'data' => $book], '200');
            ...
            if (!empty($request->razorpay_payment_id)) {
                ...
                //entry for plan payment
                    ...
                    if (isset($data) && count($data)>0) {
                        ...
                        {
                            $ref_amt=(($data['amount']*$data['ref_amount'])/100)/2;
                        }
                        else
                        {
    ...
    public function socialLogin(Request $request){
        ...
        if ($user) {
            $social=SocialLgoin::where('email',$request->email)->first();
                if($social==null){
                    SocialLgoin::insert(['email'=>$request->email,'name'=>$request->name,'profile'=>isset($request->photo) ? $request->photo : ('uploads/profile/profile.jpg'),'social_id'=>$request->social_id,'user_id'=>$user->id]);
                }
                // $user->update(['device_token' => $request->device_token]);
            ...
            if($social){
               ...
               $user_id= User::insertGetId([ 'mobile' => isset($request->mobile)?$request->mobile:null,
                ...
                'total_ref_amt' => 0,
                'photo' => isset($request->photo) ? $request->photo : ('uploads/profile/profile.jpg'),]);
                $social=SocialLgoin::insert(['email'=>$request->email,'name'=>$request->email,'profile'=>isset($request->photo) ? $request->photo : ('uploads/profile/profile.jpg'),'social_id'=>$request->social_id,'user_id'=>$user_id]);
                if($social){
                     Auth::loginUsingId($user_id);
                     Auth::user()->photo = cloudUrl(Auth::user()->photo);
                }
                 return response()->json(['status' => 200, 'msg' =>'User Login Successfully','data'=>Auth::user(),'ref_status'=>false], '200');
            }
    ...
    public function updateRef(Request $request){
        $user=User::where('id',$request->user_id)->update(['ref_code'=>$request->ref_code]);
      
        if($user>0){
            return response()->json(['status' => 200, 'msg' =>'Referal Code Successfully'], '200');
        }
        else{
            return response()->json(['status' => 201, 'msg' =>'Referal Code Not Successfully'], '201');
        }
    }
    ...
    public function recentbook(){
         $data=Book::where('status',1)->latest()->take(5)->get(['id','title','doc_type','description','orderNo','preview','file_detail','created_at','author_name']);
            $book=[];
            foreach($data as $k=>$item){
                $book[$k]=$item;
                $book[$k]['preview']=cloudUrl($item->preview);
    ...
    public function getBookById(Request $request){
         $data=Book::where('status',1)->where('title',urldecode($request->id))->first(['id','title','doc_type','description','orderNo','preview','file_detail','author_name']);
            
                
                $data->preview=cloudUrl($data->preview);
                if(isset($data->doc_type) && $data->doc_type!='text')
                {
                    $data->file_detail=cloudUrl($data->file_detail);
                }
     ...
     public function getLink(Request $request){
        ...
        $message = rawurlencode('Check out Shana, A platform where you learn to train your mind. https://play.google.com/store/apps/details?id=com.shana&hl=en_IN');
    ...
    public function getVersion(){
       $version= DB::table('version')->first();
       if ($version!=null) {
            return response()->json(['status'=>200,'version'=>$version->version],200);
        }
        else{
            return response()->json(['status' =>201,'version'=>''], 201);
        }
    }
